...
```sh
GITHUB_REPO=umair-as/mcp-netdiag-rs git-cliff --config cliff.toml --tag vX.Y.Z -o CHANGELOG.md
```
...
```sh
GITHUB_REPO=umair-as/mcp-netdiag-rs git-cliff --config cliff.toml \
...
(`feat(netdiag):`, `fix(netdiag):`, `docs:`, `test:`, `ci:`/`build:`,
`chore:`, `style:`) as this project already does. A `feat!:`/`fix!:` subject
(or any `type!:`) surfaces under a dedicated "Breaking Changes" section.
...
release workflow re-runs fmt/clippy/test/build plus the example-client build
as a final gate before tagging, so a green PR merge doesn't skip verification
at release time.
...
release binary, verifies the built binary reports `mcp-netdiag-rs X.Y.Z` via
`--version`, packages
`mcp-netdiag-rs-vX.Y.Z-x86_64-unknown-linux-gnu.tar.gz` + its `.sha256`,
...
- **Patch** (e.g. `0.1.0` → `0.1.1`): bug fixes only — no new tool
  params/fields, no behavior change for an existing caller.
- **Minor** (e.g. `0.1.0` → `0.2.0`): new backward-compatible
  functionality — a new optional field, a new tool, a new opt-in behavior
  (e.g. a new privileged tool behind `NETDIAG_ENABLE_PRIVILEGED`).
- **Major**: reserved for `1.0.0`, or a genuine breaking change to an
  existing tool's params/output shape or default behavior.
